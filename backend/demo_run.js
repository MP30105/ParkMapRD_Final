// This script requires server.js (so server runs in this process), then executes the demo steps.
...
(async () => {
  ...
  const registerRes = await fetch(base + '/api/parkmaprd/auth/register', {
      ...
      body: JSON.stringify({ email, password, name: 'Demo User', licensePlate: 'DEF-456' })
    });
